...
```
folsom-cmdp/
├── folsom_cmdp_v5.py          # Main script (env + SAC-Lag + training + eval)
├── requirements.txt           # Python dependencies
├── data/
│   └── README_data.md         # Instructions for downloading Folsom inflow data
├── checkpoints/
│   └── sac_lag_folsom_v5.pt   # Pretrained model weights (see Releases)
├── results/
│   ├── cmdp_results.png        # Training/eval figure
...
| File | Contents |
|------|---------|
| `cmdp_results_v5.png` | 6-panel training/eval figure |
| `results_summary_v5.txt` | Formatted metrics table |
| `sac_lag_folsom_v5.pt` | Saved model checkpoint |
...
ckpt = torch.load("sac_lag_folsom_v5.pt", map_location="cpu")
...
| # | Category | Fix |
|---|----------|-----|
| **FIX 1** | 🔴 CRITICAL | Action remapped to `release ∈ [0, Q_MAX]`. Agent can release below Q_MIN → eco violations are *real*. |
| **FIX 2** | Calibration | `DELTA_FRACTION=0.6`, `ECO_DELTA_FLOOR=0.001`. Thresholds are meaningful now that eco violations occur. |
| **FIX 3** | Stability | `LR_LAMBDA=0.01` with EMA smoothing (`0.9·λ_old + 0.1·λ_new`) prevents dual-variable spikes. |
| **FIX 4** | Temporal | `LAMBDA_UPDATE_FREQ=720` (one full episode). Partial updates are too noisy in seasonal environments. |
| **FIX 5** | Metrics | Violation thresholds aligned with CMDP theory: soft = rate > δ, hard = rate > 2δ. |
| **FIX 6** | Reward | Season-varying reward `r = η · H_norm · (Q_rel / Q_max) · (1 + 0.2·sin(2πt/8760))` for richer signal. |
| **FIX 7** | Retained | 30-day episodes, trimmed-mean λ update (trim top 5%), pure hydropower reward base. |
...
3. Upload `folsom_cmdp_v5.py` and run:
   ...
   !python folsom_cmdp_v5.py
   ```
4. Outputs (`cmdp_results_v5.png`, `sac_lag_folsom_v5.pt`, `results_summary_v5.txt`) appear in `/kaggle/working/`.

---

## 📦 Pretrained Checkpoint

A pretrained checkpoint trained for 300k steps is available under [**Releases**](https://github.com/<your-username>/folsom-cmdp/releases). Download `sac_lag_folsom_v5.pt` and place it in the repo root, then use the evaluation snippet above.
...
- Inflow data: [Akshat Mishra / Kaggle](https://www.kaggle.com/datasets/akshatmishra111/damnnn)
